...
(
    ...
    hyl_cavgscore10 DECIMAL(5,2) CHECK (hyl_cavgscore10 >= 0 AND hyl_cavgscore10 <= 100),
...
--计算每门课程的成绩排名
--为了实现成绩排名功能，可以创建一个函数计算每门课程的成绩排名。
-- 该函数可以在每次更新成绩时触发，确保每门课程的学生排名始终更新。

-- 计算每门课程的成绩排名
CREATE OR REPLACE FUNCTION update_course_rank()
    RETURNS TRIGGER AS $$
DECLARE
course_rank INT;
BEGIN
...
WITH ranked_students AS (
    SELECT e.hyl_sno10,
           RANK() OVER (PARTITION BY e.hyl_tcno10 ORDER BY e.hyl_escore10 DESC) AS rank
    FROM huyl_enroll10 e
    WHERE e.hyl_tcno10 = NEW.hyl_tcno10
      AND e.hyl_escore10 IS NOT NULL
)
SELECT rank INTO course_rank
FROM ranked_students
WHERE hyl_sno10 = NEW.hyl_sno10;

-- 更新学生成绩排名
UPDATE huyl_enroll10
SET hyl_escore10 = course_rank
WHERE hyl_sno10 = NEW.hyl_sno10 AND hyl_tcno10 = NEW.hyl_tcno10;
...
-- 创建触发器
CREATE TRIGGER tr_update_course_rank
    AFTER INSERT OR UPDATE ON huyl_enroll10
                        FOR EACH ROW EXECUTE FUNCTION update_course_rank();

--计算学期内课程总学分
--可以添加一个函数来计算每个学生在每学期所修的总学分。
-- 计算学期内总学分
CREATE OR REPLACE FUNCTION calculate_total_credits_for_term()
    RETURNS TRIGGER AS $$
DECLARE
total_credits DECIMAL(5,1);
BEGIN
...
WHERE e.hyl_sno10 = NEW.hyl_sno10
  AND e.hyl_escore10 >= 60  -- 只统计通过的课程
  AND e.hyl_tcyear10 = NEW.hyl_tcyear10
  AND e.hyl_tcterm10 = NEW.hyl_tcterm10;

-- 更新学生表的总学分字段
UPDATE huyl_student10
SET hyl_screditsum10 = total_credits
WHERE hyl_sno10 = NEW.hyl_sno10;
...
-- 创建触发器
CREATE TRIGGER tr_calculate_total_credits_for_term
    AFTER INSERT OR UPDATE ON huyl_enroll10
                        FOR EACH ROW EXECUTE FUNCTION calculate_total_credits_for_term();

--自动生成学生密码
--需要自动生成学生的登录密码，以下是一个简单的密码生成函数：
-- 自动生成学生密码（根据学号或姓名生成简单密码）
CREATE OR REPLACE FUNCTION generate_student_password()
    RETURNS TRIGGER AS $$
BEGIN
    NEW.hyl_upassword10 := CONCAT('stu', NEW.hyl_sno10); -- 生成密码如 "stu600001"
RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器
CREATE TRIGGER tr_generate_student_password
    BEFORE INSERT ON huyl_user10
    FOR EACH ROW EXECUTE FUNCTION generate_student_password();

--统计学生所在地区的成绩分布
-- 学生地区成绩统计视图
CREATE VIEW v_student_region_scores AS
SELECT
    s.hyl_splace10 AS student_region,
    AVG(e.hyl_escore10) AS average_score,
    COUNT(s.hyl_sno10) AS student_count
FROM huyl_enroll10 e
         JOIN huyl_student10 s ON e.hyl_sno10 = s.hyl_sno10
...
--教师教授课程的学生成绩视图
-- 教师授课学生成绩视图
CREATE VIEW v_teacher_student_scores AS
SELECT
    t.hyl_tname10 AS teacher_name,
    s.hyl_sname10 AS student_name,
    c.hyl_cname10 AS course_name,
...
-- 创建触发器
CREATE TRIGGER tr_cascade_delete_student
    AFTER DELETE ON huyl_student10
    FOR EACH ROW EXECUTE FUNCTION cascade_delete_student();
